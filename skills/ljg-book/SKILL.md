...
- Markdown 文件里图包用 fenced code block（保等宽，渲染不碎）
- 宽度 ≤80 字符
- 轴有名字，两端有极
...
3. Markdown 格式（标题用 `#`，加粗用 `**bold**` 双星号）
4. 文件头：
...
5. 正文六段：1-5 抽骨架（散文，冷），第 6 段「认知地图」（ASCII 参考系图 + 走两步预测，热）。ASCII 图包在 fenced code block 中。
6. 报告文件路径给用户。
